...
if __name__ == '__main__':
    ...
    for orbit in range(7, 13):
        for nExpo in range(13):
            ...
            exp = ExposureSet(subdf['file ID'].values, dataDIR,[subdf['YCENTER'].values[0], subdf['XCENTER'].values[0]] , orbit, nExpo, subdf['filter'].values[0], useNewFlat = 1)
            #exp.correct(correctAll = True, chisqTh = 2.5)
            exp.noUpTheRamp()
            #exp.testCorrection2(chisqTh = 3, doPlot = True, plotDIR = plotDIR)
            exp.saveFITS('../data/ABPIC-B_noramp_newflat/', decorator = 'noramp')
            exp.savePickle('../data/ABPIC-B_noramp_newflat/', decorator = 'noramp')
            exp.save('../data/ABPIC-B_noramp_newflat/', 'noramp_expSet')