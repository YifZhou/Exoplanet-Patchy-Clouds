...
if __name__ == '__main__':
    ...
    axes[0].hist(x_gcen - x_cen, bins = xbins, label = 'gcentroid', alpha = 0.5)
    axes[0].set_ylabel('Gcentroid')
    axes[1].hist(x_corr - x_cen, bins = xbins, label = 'crosscorr', alpha = 0.5)
    axes[1].set_ylabel('Crosscorr')
    axes[2].hist(x_wcs - x_cen, bins = xbins, label = 'WCS', alpha = 0.5)
...
#    ax_y.plot(obs_num, 0*obs_num, label = 'centroid')
    axes[0].hist(y_gcen - y_cen, bins = xbins, label = 'gcentroid', alpha = 0.5)
    axes[0].set_ylabel('Gcentroid')
    axes[1].hist(y_corr - y_cen, bins = xbins, label = 'crosscorr', alpha = 0.5)
    axes[1].set_ylabel('Crosscorr')
    axes[2].hist(y_wcs - y_cen, bins = xbins, label = 'WCS', alpha = 0.5)
    axes[2].set_ylabel('WCS')
