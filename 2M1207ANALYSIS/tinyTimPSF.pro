...
FUNCTION shiftPSF, psf0, dx, dy, factor = factor
  ...
  ;; so the subsize will be 27*9 = 243 and 243/2 = 121
  psf = psf[peak_xy - 121: peak_xy + 121, peak_xy - 121: peak_xy + 121]

  ;;Step Three, rebin the psf, using frebin
  psf = frebin(psf, 27, 27, /total)
...
FUNCTION register2PSFs, im, PSF1, PSF02, mask, gc, weight = weight
  ...
  WHILE gridsize GT 1e-2 DO BEGIN
     ...
     FOR i=0,ngrid-1 DO BEGIN
        FOR j=0,ngrid-1 DO BEGIN
           PSF2 = shiftPSF(PSF02, (gridx[i]-13), (gridy[j]-13), factor = 10)
...
function PSFPhotometry1, fn, filterName, angle, dither, xy0
  ...
  FOR i = 0, nPSFs - 1 DO begin
     ...
     xyList[*, i] = xy
     PSF= shiftPSF(psf0, dxy[0], dxy[1], factor = 10)
     mask2 = make_mask(mask, [[xy[0], xy[1], 0, 3.0], [comp_xy[0], comp_xy[1], 0, 3], [xy[0], xy[1], 11, 100]])
     opt_paras = fit1PSF(im, PSF, mask2, weight = 1/err^2)
     PSFList[*, *, i] = PSF
  ...
  mask40 = fltarr(27, 27)
  mask40[11:26,0:15] = 1 ;;; only calculate the fourth quadrant
  mask4 = make_mask(mask, [[[xy[0], xy[1], 0, 3.5], [xy[0], xy[1], 11, 100]]]);;*mask40
  ;;mask4 = make_mask(mask, [[xy[0], xy[1], 11, 100]])*mask40
  comp_xy = register2PSFs(im, PSF1, PSF02, mask4, comp_xy, weight = 1/err^2)
  PSF2 = shiftPSF(psf02, comp_xy[0] - 13, comp_xy[1] - 13, factor = 10)
  amps = fit2PSFs(im, PSF1, PSF2, mask4, weight = 1/err^2)
...
PRO tinytimPSF
  ...
  FOR i=0, N_elements(F125ID) - 1 DO BEGIN
     id = F125ID[i]
     a = PSFPhotometry1(F125Info.filename[id], F125Info.filter[id], long(F125Info.PosAngle[id]), long(F125Info.dither[id]), xy[*, long(F125Info.dither[id]), long(F125Info.posAngle[id])])
     fluxA[i] = a[0]
     fluxB[i] = a[1]
     sky[i] = a[2]
     chisq[i] = a[3]
