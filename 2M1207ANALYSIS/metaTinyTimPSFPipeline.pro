;;;;; a data reduction pipeline including residual generation and PSF fit
...
function PSFPhotometry, fn, filterName, angle, dither, xy0, removeResidual=removeResidual
  ...
  ;; ENDELSE
  IF removeResidual THEN BEGIN
     residual0 = residual[*, *, angle*4 + dither] 
     im = im - residual0 ;; remove the difference of residual and PSF
  ENDIF
  ...
  spawn, 'python PSF_generate_list.py ' + strn(xy0[0]) + ' ' + strn(xy0[1])$
         + ' ' + primaryTTFN + ' ' + ' ' + strn(MJD) + ' 0'  ;; use tinytim to generate a PSF file that works for  companion
  ...
  spawn, 'python PSF_generator.py ' + strn(comp_xy0[0]) + ' ' + strn(comp_xy0[1])$
         ...
         + ' ' + strn(MJD) + ' comp_PSF 0' ;; use tinytim to generate a PSF file that works for  companion
  ...
  ENDIF ELSE BEGIN
     writefits, './fitsResidual/'+strmid(fn, 0, 9) + '.fits', im
     writefits, './fitsResidual/'+strmid(fn, 0, 9) + '.fits', PSF1*amps[0] + PSF2*amps[1]+amps[2], /append
  ENDELSE
...
PRO tinytimPSF, removeResidual, residualFN
  ...
  IF removeResidual THEN restore, residualFN[0]
  ...
  FOR i=0, N_elements(F125ID) - 1 DO BEGIN
     id = F125ID[i]
     a = PSFPhotometry(F125Info.filename[id], F125Info.filter[id], long(F125Info.PosAngle[id]), long(F125Info.dither[id]), xy[*, long(F125Info.dither[id]), long(F125Info.posAngle[id])], removeResidual=removeResidual)
     fluxA[i] = a[0]
     fluxB[i] = a[1]
     sky[i] = a[2]
     chisq[i] = a[3]
  ...
  IF removeResidual THEN restore, residualFN[1]
  xy = [[[135,161], [145,161], [135,173], [145,173]],$
      [[142, 159],[152,159], [142, 171], [152, 171]]]
  ...
  FOR i=0, N_elements(F160ID) - 1 DO BEGIN
     id = F160ID[i]
     a = PSFPhotometry(F160Info.filename[id], F160Info.filter[id], long(F160Info.PosAngle[id]), long(F160Info.dither[id]), xy[*, long(F160Info.dither[id]), long(F160Info.posAngle[id])], removeResidual=removeResidual)
     fluxA[i] = a[0]
     fluxB[i] = a[1]
     sky[i] = a[2]
     chisq[i] = a[3]
...
function combineResidual
  ...
  FOREACH angle, [0, 1] DO BEGIN
    FOREACH dither, [0, 1, 2, 3] DO BEGIN
       ...
       FOR i = 0, n_elements(IDList) - 1 DO BEGIN
          id = IDList[i]
          fn = './fitsResidual/' + strmid(F125INFO.filename[id], 0, 9) + '.fits' ;;;saved fitting result fits file
    ...
    ENDFOREACH
    ENDFOREACH
    residualFN[0] = dateString()+'F125W_residual.sav'
  save, residual, file = residualFN[0]
  ...
  FOREACH angle, [0, 1] DO BEGIN
     FOREACH dither, [0, 1, 2, 3] DO BEGIN
       ...
       FOR i = 0, n_elements(IDList) - 1 DO BEGIN
          id = IDList[i]
          fn = './fitsResidual/' + strmid(F160INFO.filename[id], 0, 9) + '.fits' ;;;saved fitting result fits file
          im = mrdfits(fn, 0, /silent)
          psf = mrdfits(fn, 1, /silent)
          residualCube[*, *, i] = im - psf ;;;save the residual for every individual exposure
       ENDFOR
